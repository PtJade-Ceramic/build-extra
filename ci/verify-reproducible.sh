	...
	then
		...
		*)
			...
			then
				...
				for sec in .rsrc .rdata .reloc .debug
				do
					...
					then
						...
						then
						...
						fi							# The .debug section carries the CodeView/PDB reference.
							# Its GUID is generated by the linker and may be the
							# only non-reproducible bytes left; show both sides.
							if test "$sec" = .debug && type -p python >/dev/null 2>&1 \
								&& test -f "$(dirname "$0")/pe-debug.py"
							then
								for side in a b
								do
									if test "$side" = a; then exe_f="$extract_a"; else exe_f="$extract_b"; fi
							...
							# The .reloc table is the base-relocation list; pinpoint
							# which block holds the first differing byte.
							if test "$sec" = .reloc && type -p python >/dev/null 2>&1 \
								&& test -f "$(dirname "$0")/pe-reloc.py"
							then
								if test -n "$off"
								then
									at=$((off - 1))
								...
								do
									if test "$side" = a; then exe_f="$extract_a"; else exe_f="$extract_b"; fi
									echo "  [$side] .reloc (first diff at $at):" >&2
									python "$(dirname "$0")/pe-reloc.py" --at "$at" "$exe_f" >&2 2>/dev/null \
										| sed -n '1,20p' || true
								done
							fi					fi
