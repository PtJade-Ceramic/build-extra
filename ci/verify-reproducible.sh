	...
	then
		...
		*)
			...
			then
				...
				# .reloc and .debug cannot always be extracted from a PE
				# with objcopy, so read them directly with python instead:
				# attribute the first differing byte to a section, show
				# the CodeView/PDB reference (GUID) and locate the
				# difference in the base-relocation table.
				if type -p python >/dev/null 2>&1 \
					&& test -f "$(dirname "$0")/pe-segments.py"
				then
					off=$(cmp -l "$extract_a" "$extract_b" 2>/dev/null |
						sed -n '1s/[[:space:]].*//p')
					echo "  [segments] first diff at: ${off:-?}" >&2
					python "$(dirname "$0")/pe-segments.py" "$extract_a" "${off:-0}" >&2 2>/dev/null || true
					python "$(dirname "$0")/pe-segments.py" "$extract_b" "${off:-0}" >&2 2>/dev/null || true
					if test -f "$(dirname "$0")/pe-debug.py"
					then
						for side in a b
						do
							if test "$side" = a; then exe_f="$extract_a"; else exe_f="$extract_b"; fi
							echo "  [$side] .debug:" >&2
							python "$(dirname "$0")/pe-debug.py" "$exe_f" >&2 2>/dev/null || true
						done
					fi
					if test -n "$off" && test -f "$(dirname "$0")/pe-reloc.py"
					then
						at=$((off - 1))
						python "$(dirname "$0")/pe-reloc.py" --at "$at" "$extract_a" >&2 2>/dev/null \
							| sed -n '1,16p' || true
					fi
				fi
			fi
