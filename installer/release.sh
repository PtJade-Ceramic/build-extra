...
# ---- Reproducible: uniformly pin the mtime of every file to be packaged ----
# Inno Setup writes file timestamps into the installer; any mtime difference -> different installer bytes.
# SOURCE_DATE_EPOCH is exported by CI (build-localized.sh); skipped for local builds without it.
# This covers the make-file-list Sources plus the hard-coded Sources in release.sh/install.iss
# (including ReleaseNotes.html and package-versions.txt generated by this script, which cannot be
# touched ahead of time on the CI side).
if test -n "$SOURCE_DATE_EPOCH"; then
	echo "==> pinning packaged-file mtimes (SOURCE_DATE_EPOCH=$SOURCE_DATE_EPOCH)"
	# 1) Sources in file-list.iss (with/without quotes; skip {#SourcePath} pointing at the build-extra side)
	perl -ne 'if (/^Source: (?:"([^"]+)"|([^;]+));/) { print (defined $1 ? $1 : $2), "\n" }' file-list.iss |
	while IFS= read -r s
	do
		case "$s" in
		\{#SourcePath\}*) ;;
		*) s="${s//\\//}"; test -e "/$s" && touch -d "@$SOURCE_DATE_EPOCH" "/$s" 2>/dev/null ;;
		esac
	done
	# 2) Fixed Sources on the build-extra/installer side (including files generated by this script)
	for fx in ReleaseNotes.html package-versions.txt NOTICE.txt \
		../ReleaseNotes.css ../post-install.bat ../LICENSE.txt ../aslr-manager.ps1
	do
		test -e "$fx" && touch -d "@$SOURCE_DATE_EPOCH" "$fx" 2>/dev/null
	done
fi
