...
# This covers the make-file-list Sources plus the hard-coded Sources in
# release.sh/install.iss (including ReleaseNotes.html and package-versions.txt
# generated by this script, which cannot be touched ahead of time).
if test -n "$SOURCE_DATE_EPOCH"; then
	echo "==> pinning packaged-file mtimes (SOURCE_DATE_EPOCH=$SOURCE_DATE_EPOCH)"
	# 1) Sources in file-list.iss and install.iss (with/without quotes;
	...
	do
		...
		do
			...
			\{#SourcePath\}*)
				s="${s#\{#SourcePath\}*}"
				s="${s#\\}"; s="${s#/}"
				test -e "$s" && touch -d "@$SOURCE_DATE_EPOCH" "$s" 2>/dev/null
				;;
			*) s="${s//\\//}"; test -e "/$s" && touch -d "@$SOURCE_DATE_EPOCH" "/$s" 2>/dev/null ;;
			esac
		done
	done
	# 2) Fixed Sources on the build-extra/installer side (including generated files)
